...
function Goal({ goal, setGoals, globalHashTags }) {
    ...
    useEffect(() => {
        let myInterval = setInterval(() => {
            if (goal.isActive) {
                setDisplaySeconds(
                    (prevDisplaySeconds) => prevDisplaySeconds + 1
                );
            }
        }, 1000);
        return () => {
    ...
    const handleStartPause = () => {
        setGoals((prevGoals) => {
            return prevGoals.map((prevGoal) => {
                ...
                return {
                    ...prevGoal,
                    latestStartTimeStamp: !prevGoal.isActive
                        ? new Date()
                        : null,
                    ...(prevGoal.isActive && {
                        duration:
    ...
    return (
        <Paper className={classes.root} elevation={2}>
            ...
            <Typography variant="overline" component="p" gutterBottom>
                {" "}
                You have spent{" "}
                {humanizeDuration(displaySeconds * 1000, { round: true })} on
                this task
            </Typography>
            {goal?.hashTags &&
